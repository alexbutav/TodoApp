<?php
    ...
    class HomeController extends Core\Controller {
        protected $sort_field, $sort_direction;
        ...
        public function index($page_id = 1) {
            $items_pre_page = 3;
            $tasks = Task::orderBy($this->sort_field, $this->sort_direction)->paginate($items_pre_page, ['id', 'name', 'email', 'body', 'done', 'edited'], 'page', $page_id);
            Core\View::render("home/index.html", ['current_page' => $page_id, 'tasks' => $tasks]);
        }

        public function changeSortField($field = 'id') {
            if( !in_array($field, ['id', 'name', 'email'])) $this->returnWithMessages("Недопустимое поле сортировки!", "/");
        ...
        public function changeSortDirection($direction = 'asc') {
            if( !in_array($direction, ['asc', 'desc'])) $this->returnWithMessages("Недопустимое направление сортировки!", "/");
