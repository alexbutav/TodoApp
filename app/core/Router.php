<?php
    ...
    class Router {
        ...
        protected 
            $url,                      // 
            $params            = [],   // параметры полученые из URI
            $routes            = [],   // таблица маршрутизации
            $controller        = "",   // контроллер
            $controller_action = "",   // метод контроллера
            $action            = NULL; // переданное замыкание
        ...
        public function add($route, $callback) {
            ...
            if( is_string($callback) and !is_numeric($callback)) {
            ...
            foreach( $this->routes as $route => $props ) {
                if( preg_match($route, $this->url, $matches) ) {
                    ...
                    foreach ($matches as $key => $match) {
                        if( is_string($key) and !empty($match)) {
                            $this->params[$key] = $match;
                        }
                    }

                    // запоминаем actions соответствующие маршруту
                    ['controller' => $this->controller, 'action' => $this->controller_action, 'function' => $this->action] = $props;
